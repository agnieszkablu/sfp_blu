...
function sfp_setup() {
	...
	register_nav_menus( array(
		...
		'language-switching' => esc_html__( 'Language switching', 'sfp' )
	) );
...
function sfp_scripts() {
	...
	if( !is_admin()){
		...
		wp_register_script('jquery', get_template_directory_uri().'/bower_components/jquery/dist/jquery.min.js', false, '1.0.0', true);
	...
	//Enqueue scripts and styles to front page only
 	if ( is_front_page() ) {
		wp_enqueue_script('aoscript', get_template_directory_uri().'/js/aos.min.js', array(), '1.0.0', false);
        ...
        global $is_IE;
        if( $is_IE ) {
            wp_enqueue_script('scriptsfront', get_template_directory_uri().'/js/scripts-front1.js', array(), '1.0.0', true);
        }else{
